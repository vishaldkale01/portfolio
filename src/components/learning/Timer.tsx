...
export default function Timer({ taskId, activeTimer: initialTimer, totalTimeSpent }: TimerProps) {
  ...
  return (
    <div className="flex items-center gap-2">
      <div className={`flex items-center gap-1.5 px-2 py-1 rounded-md border ${isRunning
        ? 'bg-blue-500/10 border-blue-500/30'
        : 'bg-gray-700/30 border-gray-600/30'
        }`}>
        <span className={`w-1.5 h-1.5 rounded-full ${isRunning ? 'bg-blue-400 animate-pulse' : 'bg-gray-500'}`} />
        <span className={`font-mono  rounded-full font-medium ${isRunning ? 'text-blue-400' : 'text-gray-400'}`}>
      ...
      <button
        ...
        disabled={loading}
        className={`p-1.5 rounded-md transition-all duration-200 border ${isRunning
      ...
      >
        {loading ? (
          ...
          <svg className="w-3.10 h-4" fill="currentColor" viewBox="0 0 24 24">
